...
## Project specifics: agentic-llama.cpp-prism

This host carries the port of Prism ML's ternary GGUF types (PTQ1_0, PQ2_0) to
the x86 SIMD ladder of the CPU backend, so Ternary-Bonsai-2-27B runs on a
...
- Primary: i9-10885H (Comet Lake, AVX2+FMA, no VNNI) + 4 GiB Turing CUDA GPU
  (TU117 class), PCIe Gen3 x16.
- Stretch: i7-2600K (Sandy Bridge, AVX only, no FMA), i7-4670K (Haswell,
  AVX2+FMA), plus Zen 1-4 as the divergence classes feature detection must
  separate. Agner Fog's instruction tables (pinned under `refs/agner/`, cited
...
a scope prefix). Host commits follow the hook-gated style. Pin bumps in
`.host-software` happen only after the software commit is pushed. Kernels are
benchmarked with `llama-bench` before and after; the numbers land in the plan
milestone with the command line that produced them.
